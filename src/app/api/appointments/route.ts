...
export async function GET(request: Request) {
  try {
    ...
    if (patient) queryParams.append("patient", `Patient/${patient}`);
    if (date) queryParams.append("date", date);
    if (provider) queryParams.append("practitioner", `Practitioner/${provider}`);

    const query = queryParams.toString();
    if (!query) {
      return NextResponse.json(
        { error: "Missing query parameters (patient, date, or provider)" },
        { status: 400 }
      );
    }

    const modmedUrl = `${BASE_URL}/fhir/v2/Appointment?${query}`;
    ...
    if (!response.ok) {
      ...
      console.error(`ModMed Appointment GET error: ${errorMessage}`);
      return NextResponse.json(
        { error: errorMessage },
